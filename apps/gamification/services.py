...
def _award_xp(enrollment, event_type, xp, source_key, attempt=None, **metadata) -> bool:
...
def activity_days(learner: LearnerProfile) -> set:
    """Calendar days (Django's time zone) with at least one judged attempt, in any course."""
    return set(
...
def sync_learner(learner: LearnerProfile, attempt: ExerciseAttempt | None = None) -> list:
    ...
    with transaction.atomic():
        ...
        profile.save()
    return _sync_achievements(learner, profile, attempt)


def _sync_achievements(learner, profile, attempt) -> list:
    ...
    for achievement in Achievement.objects.filter(code__in=pending, is_active=True):
        if attempt is not None:
            context = {
                ...
                "world_id": attempt.enrollment.world_id,
                "attempt": attempt,
            }
        else:
